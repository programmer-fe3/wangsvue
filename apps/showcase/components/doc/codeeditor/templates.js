...
const core_dependencies = {
    ...
    vite: '^4.0.0',
    wangsvue: pkg.version || wangsVue.version || 'latest',
    '@wangsvue/themes': pkg.version || wangsVue.version || 'latest',
    '@wangsvue/auto-import-resolver': pkg.version || wangsVue.version || 'latest',
    wangsicons: app_dependencies['wangsicons'] || 'latest',
...
const getVueApp = (props = {}, sourceType) => {
    const path = 'src/';
    const { code: sources, title = 'wangsvue_demo', description = '', service, extPages, dependencies: deps, component, extFiles } = props;
    ...
    if (deps !== null && component !== null) {
        imports += `import ${component} from 'wangsvue/${component.toLowerCase()}';
`;
    ...
    const files = {
        'package.json': {
            content: {
                ...
                description: `**${description}** ${wangsVue.description}`,
                keywords: [],
                scripts: {
                    dev: 'vite',
                    build: 'vite build',
...
<html lang="en">
    <head>
        ...
        <meta name="description" content="**${description}** ${wangsVue.description}" />
        <!-- Added to show icons in the editor -->
        <link rel="stylesheet" href="https://unpkg.com/wangsicons@${dependencies['wangsicons'].replace(/[\^|~]/gi, '')}/wangsicons.css">
        <title>wangsVue App</title>
    </head>
    <body>
        <div id="app"></div>
...
import ConfirmationService from 'wangsvue/confirmationservice'
import DialogService from 'wangsvue/dialogservice'
import ToastService from 'wangsvue/toastservice';
...
<script>
import { $t, updatePreset, updateSurfacePalette } from '@wangsvue/themes';
import Aura from '@wangsvue/themes/aura';
import Lara from '@wangsvue/themes/lara';
import Nora from '@wangsvue/themes/nora';
...
export default {
    ...
    methods: {
        ...
        onRippleChange(value) {
            this.$wangsvue.config.ripple = value;
        },
    ...
    computed: {
        rippleActive() {
            return this.$wangsvue.config.ripple;
        }
    }
};
<\/script>
`
        ...
        [`${path}presets/Noir.js`]: {
            content: `import { definePreset } from '@wangsvue/themes';
        import Aura from '@wangsvue/themes/aura';
