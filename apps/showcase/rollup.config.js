...
function addCore() {
    ENTRY.format.es({ input: process.env.INPUT_DIR + 'config/WangsVue.js', output: process.env.OUTPUT_DIR + 'config/config' });
    ENTRY.format.es({ input: process.env.INPUT_DIR + 'service/WangsVueService.js', output: process.env.OUTPUT_DIR + 'service/WangsVueService' });
}
...
function addLibrary() {
    THEME_PRESETS?.forEach((preset) => {
        ENTRY.format.umd({ name: `wangsVue.Themes.${preset[0].toUpperCase() + preset.slice(1)}`, input: process.env.INPUT_DIR + `themes/${preset}/index.js`, output: process.env.OUTPUT_DIR + `umd/themes/${preset}`, minify: true });
    });
...
function addPackageJson() {
    ...
    "description": "wangsVue is an open source UI library for Vue featuring a rich set of 80+ components, a theme designer, various theme alternatives such as Material, Bootstrap, Tailwind, premium templates and professional support. In addition, it integrates with wangsBlock, which has 370+ ready to use UI blocks to build spectacular applications in no time.",
    ...
    "unpkg": "umd/wangsvue.min.js",
    "jsdelivr": "umd/wangsvue.min.js",
