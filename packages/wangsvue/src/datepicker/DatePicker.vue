    ...
    <span ref="container" :id="d_id" :class="cx('root')" :style="sx('root')" v-bind="ptmi('root')">
        ...
        <slot v-if="showIcon && iconDisplay === 'button' && !inline" name="dropdownbutton">
            <button
                ...
                @click="onButtonClick"
                type="button"
                :aria-label="$wangsvue.config.locale.chooseDate"
                aria-haspopup="dialog"
                :aria-expanded="overlayVisible"
                :aria-controls="panelId"
                v-bind="ptm('dropdown')"
                            ...
                            <div v-for="(month, groupIndex) of months" :key="month.month + month.year" :class="cx('calendar')" v-bind="ptm('calendar')">
                                <div :class="cx('header')" v-bind="ptm('header')">
                                    ...
                                    <Button
                                        ...
                                        :disabled="disabled"
                                        :aria-label="currentView === 'year' ? $wangsvue.config.locale.prevDecade : currentView === 'month' ? $wangsvue.config.locale.prevYear : $wangsvue.config.locale.prevMonth"
                                        :unstyled="unstyled"
                                    ...
                                    <div :class="cx('title')" v-bind="ptm('title')">
                                        <template v-if="$wangsvue.config.locale.showMonthAfterYear">
                                            <button
                                                ...
                                                :aria-label="$wangsvue.config.locale.chooseYear"
                                            ...
                                            <button
                                                ...
                                                :disabled="switchViewButtonDisabled"
                                                :aria-label="$wangsvue.config.locale.chooseMonth"
                                    ...
                                    <Button
                                        ...
                                        :disabled="disabled"
                                        :aria-label="currentView === 'year' ? $wangsvue.config.locale.nextDecade : currentView === 'month' ? $wangsvue.config.locale.nextYear : $wangsvue.config.locale.nextMonth"
                                        :unstyled="unstyled"
                    ...
                    <div v-if="(showTime || timeOnly) && currentView === 'date'" :class="cx('timePicker')" v-bind="ptm('timePicker')">
                        <div :class="cx('hourPicker')" v-bind="ptm('hourPicker')" data-pc-group-section="timepickerContainer">
                            <Button
                                ...
                                :aria-label="$wangsvue.config.locale.nextHour"
                                :unstyled="unstyled"
                            ...
                            <Button
                                ...
                                :aria-label="$wangsvue.config.locale.prevHour"
                                :unstyled="unstyled"
                                @mousedown="onTimePickerElementMouseDown($event, 0, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                                @keydown="onContainerButtonKeydown"
                        ...
                        <div :class="cx('minutePicker')" v-bind="ptm('minutePicker')" data-pc-group-section="timepickerContainer">
                            <Button
                                :class="cx('pcIncrementButton')"
                                :aria-label="$wangsvue.config.locale.nextMinute"
                                :disabled="disabled"
                                :unstyled="unstyled"
                            ...
                            <Button
                                :class="cx('pcDecrementButton')"
                                :aria-label="$wangsvue.config.locale.prevMinute"
                                :disabled="disabled"
                                @mousedown="onTimePickerElementMouseDown($event, 1, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                                @keydown="onContainerButtonKeydown"
                        ...
                        <div v-if="showSeconds" :class="cx('secondPicker')" v-bind="ptm('secondPicker')" data-pc-group-section="timepickerContainer">
                            <Button
                                :class="cx('pcIncrementButton')"
                                :aria-label="$wangsvue.config.locale.nextSecond"
                                :disabled="disabled"
                                :unstyled="unstyled"
                            ...
                            <Button
                                :class="cx('pcDecrementButton')"
                                :aria-label="$wangsvue.config.locale.prevSecond"
                                :disabled="disabled"
                                :unstyled="unstyled"
                                @mousedown="onTimePickerElementMouseDown($event, 2, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                        ...
                        <div v-if="hourFormat == '12'" :class="cx('ampmPicker')" v-bind="ptm('ampmPicker')">
                            <Button
                                :class="cx('pcIncrementButton')"
                                :aria-label="$wangsvue.config.locale.am"
                                :disabled="disabled"
                                :unstyled="unstyled"
                            ...
                            <span v-bind="ptm('ampm')" data-pc-group-section="timepickerlabel">{{ pm ? $wangsvue.config.locale.pm : $wangsvue.config.locale.am }}</span>
                            <Button
                                :class="cx('pcDecrementButton')"
                                :aria-label="$wangsvue.config.locale.pm"
                                :disabled="disabled"
                                @click="toggleAMPM($event)"
                                @keydown="onContainerButtonKeydown"
                                v-bind="timepickerButtonProps"
...
import { ConnectedOverlayScrollHandler, UniqueComponentId } from '@wangsvue/core/utils';
import CalendarIcon from '@wangsvue/icons/calendar';
import ChevronDownIcon from '@wangsvue/icons/chevrondown';
import ChevronLeftIcon from '@wangsvue/icons/chevronleft';
import ChevronRightIcon from '@wangsvue/icons/chevronright';
import ChevronUpIcon from '@wangsvue/icons/chevronup';
import Button from 'wangsvue/button';
import InputText from 'wangsvue/inputtext';
import OverlayEventBus from 'wangsvue/overlayeventbus';
import Portal from 'wangsvue/portal';
import Ripple from 'wangsvue/ripple';
...
export default {
    ...
    methods: {
        ...
        onOverlayEnter(el) {
            ...
            if (this.autoZIndex) {
                ZIndex.set('overlay', el, this.baseZIndex || this.$wangsvue.config.zIndex.overlay);
            }
                    ...
                    if (literal) {
                        ...
                        switch (format.charAt(iFormat)) {
                            case 'd':
                                ...
                                break;
                            case 'D':
                                output += formatName('D', date.getDay(), this.$wangsvue.config.locale.dayNamesShort, this.$wangsvue.config.locale.dayNames);
                                break;
                            case 'o':
                            ...
                            case 'm':
                                ...
                                break;
                            case 'M':
                                output += formatName('M', date.getMonth(), this.$wangsvue.config.locale.monthNamesShort, this.$wangsvue.config.locale.monthNames);
                                break;
                            case 'y':
        ...
        formatTime(date) {
            ...
            if (this.hourFormat === '12') {
                output += date.getHours() > 11 ? ` ${this.$wangsvue.config.locale.pm}` : ` ${this.$wangsvue.config.locale.am}`;
            }
        ...
        populateTime(value, timeString, ampm) {
            ...
            this.pm = ampm === this.$wangsvue.config.locale.pm || ampm === this.$wangsvue.config.locale.pm.toLowerCase();
                ...
                if (literal) {
                    ...
                    switch (format.charAt(iFormat)) {
                        case 'd':
                            ...
                            break;
                        case 'D':
                            getName('D', this.$wangsvue.config.locale.dayNamesShort, this.$wangsvue.config.locale.dayNames);
                            break;
                        case 'o':
                            doy = getNumber('o');
                            break;
                        case 'm':
                            ...
                            break;
                        case 'M':
                            month = getName('M', this.$wangsvue.config.locale.monthNamesShort, this.$wangsvue.config.locale.monthNames);
                            break;
                        case 'y':
                            year = getNumber('y');
                            break;
        ...
        getMonthName(index) {
            return this.$wangsvue.config.locale.monthNames[index];
        },
        getYear(month) {
        ...
        createResponsiveStyle() {
            if (this.numberOfMonths > 1 && this.responsiveOptions && !this.isUnstyled) {
                if (!this.responsiveStyleElement) {
                    ...
                    setAttribute(this.responsiveStyleElement, 'nonce', this.$wangsvue?.config?.csp?.nonce);
    ...
    computed: {
        ...
        weekDays() {
            let weekDays = [];
            let dayIndex = this.$wangsvue.config.locale.firstDayOfWeek;

            for (let i = 0; i < 7; i++) {
                weekDays.push(this.$wangsvue.config.locale.dayNamesMin[dayIndex]);
        ...
        sundayIndex() {
            return this.$wangsvue.config.locale.firstDayOfWeek > 0 ? 7 - this.$wangsvue.config.locale.firstDayOfWeek : 0;
        },
        datePattern() {
            return this.dateFormat || this.$wangsvue.config.locale.dateFormat;
        },
        monthPickerValues() {
            ...
            for (let i = 0; i <= 11; i++) {
                monthPickerValues.push({ value: this.$wangsvue.config.locale.monthNamesShort[i], selectable: isSelectableMonth(i) });
            }
        ...
        todayLabel() {
            return this.$wangsvue.config.locale.today;
        },
        clearLabel() {
            return this.$wangsvue.config.locale.clear;
        },
        weekHeaderLabel() {
            return this.$wangsvue.config.locale.weekHeader;
        },
        monthNames() {
            return this.$wangsvue.config.locale.monthNames;
        },
        attributeSelector() {
