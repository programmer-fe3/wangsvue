...
import UploadIcon from '@wangsvue/icons/upload';
import Button from 'wangsvue/button';
import Message from 'wangsvue/message';
import ProgressBar from 'wangsvue/progressbar';
...
export default {
    ...
    methods: {
        ...
        formatSize(bytes) {
            const k = 1024;
            const dm = 3;
            const sizes = this.$wangsvue.config.locale?.fileSizeTypes || ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    ...
    computed: {
        ...
        basicFileChosenLabel() {
            if (this.auto) return this.chooseButtonLabel;
                ...
                return this.$wangsvue.config.locale?.fileChosenMessage?.replace('{0}', this.files.length);
            }

            return this.$wangsvue.config.locale?.noFileChosenMessage || '';
        },
        hasFiles() {
        ...
        chooseButtonLabel() {
            return this.chooseLabel || this.$wangsvue.config.locale.choose;
        },
        uploadButtonLabel() {
            return this.uploadLabel || this.$wangsvue.config.locale.upload;
        },
        cancelButtonLabel() {
            return this.cancelLabel || this.$wangsvue.config.locale.cancel;
        },
        completedLabel() {
            return this.$wangsvue.config.locale.completed;
        },
        pendingLabel() {
            return this.$wangsvue.config.locale.pending;
        }
    },
    components: {
        Button,
