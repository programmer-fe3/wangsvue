...
import { components, composables, directives } from '@wangsvue/metadata';
    ...
    return [
        {
            name: 'wangsVue',
            as: 'wangsVue',
            from: resolvePath({ name: 'wangsVue', as: 'wangsVue', from: `wangsvue/config`, type: 'config' })
        }
    ];
}
...
function registerServices(resolvePath: any, registered: any) {
    ...
    return [...services].map((service) => ({
        name: service,
        as: service,
        from: resolvePath({ name: service, as: service, from: `wangsvue/${service.toLowerCase()}`, type: 'service' })
    }));
}

function registerStyles(resolvePath: any, registered: any, moduleOptions: ModuleOptions) {
    const options: wangsVueConfiguration = moduleOptions.options || {};

    const styles: MetaType[] = [
        {
            name: 'BaseStyle',
            as: 'BaseStyle',
            from: resolvePath({ name: 'BaseStyle', as: 'BaseStyle', from: '@wangsvue/core/base/style', type: 'style' })
        }
    ];
    ...
    if (!moduleOptions.autoImport && !options?.unstyled) {
        if (isNotEmpty(registered?.components)) {
            styles.push({
                ...
                from: resolvePath({ name: 'BaseComponentStyle', as: 'BaseComponentStyle', from: '@wangsvue/core/basecomponent/style', type: 'style' })
            });
        }
        ...
        [registered.components, registered.directives]
            ...
            .forEach((item: any) =>
                styles.push({
                    name: `${item.as}Style`,
                    as: `${item.as}Style`,
                    from: resolvePath({ name: `${item.as}Style`, as: `${item.as}Style`, from: `wangsvue/${item.as.toLowerCase()}/style`, type: 'style' })
                })
            );
    }
